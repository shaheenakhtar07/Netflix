...
const Movie = () => {
    ...
    function fetchTrailer(id){
        fetch(`http://api.themoviedb.org/3/movie/${id}/videos?api_key=0b1096e1f53fa88bb9da8d5afd7a7f44`)
        .then(res=>res.json())
        .then(d=>setTrailer(d.results[0].key))
    }

    function fetchCast(id){
        fetch(`https://api.themoviedb.org/3/movie/${id}/credits?api_key=0b1096e1f53fa88bb9da8d5afd7a7f44`)
        .then(res=>res.json())
    ...
    <div className="cast" style={{textAlign:'center'}}>
        <h1>Cast</h1>
        {
            cast.slice(0,10).map((movie)=>{
                return(
                    <img onClick={()=>{navigate('/cast',{state:{movie}})}} style={{margin:'10px',borderRadius:'50%'}} key={movie.id} height={'100px'} width={'100px'} src={`${baseUrl}${movie.profile_path}`}></img>
                )
            })
        }
    </div>
